...
and the junk drawer of odd lines in SDCI's permit-fee billing data.
...
The dataset reaches back to 2005; this page analyzes invoices from 2020-01-01
through the latest invoice date in the file (computed, never hardcoded). The
full history is used only to decide which fee descriptions are truly new.

The dataset has no balance snapshot and no void flag: amount_due is computed
(feeamount minus feeamountpaid) and includes voided or reissued invoice lines.
Every dollar figure here counts amount_paid only, so reissues cannot inflate
totals; the same-day-repeat check counts paid lines only for the same reason.

Output: src/lib/generated/fees-surcharges.json.
"""
...
df_all["record_id"] = df_all["record_id"].fillna("")
df_all["amount_paid"] = pd.to_numeric(df_all["amount_paid"], errors="coerce").fillna(0)
df_all["amount_due"] = pd.to_numeric(df_all["amount_due"], errors="coerce").fillna(0)
df_all["billed"] = df_all["amount_paid"] + df_all["amount_due"]
df_all["dt"] = pd.to_datetime(df_all["date_invoiced"], errors="coerce")
df_all["year"] = df_all["dt"].dt.year
df_all["day"] = df_all["dt"].dt.date
# Some descriptions carry a trailing space; strip for matching.
df_all["desc"] = df_all["description"].fillna("").str.strip()

# The dataset reaches back to 2005; this page's analysis window starts 2020.
ANALYSIS_START = pd.Timestamp("2020-01-01")
df = df_all[df_all["dt"] >= ANALYSIS_START]

out = {"generatedAt": datetime.now(timezone.utc).isoformat()}
out["windowStart"] = str(df["dt"].min().date())
out["windowEnd"] = str(df["dt"].max().date())
out["datasetFirstDate"] = str(df_all["dt"].min().date())
...
tech_launch = tech["dt"].min()
sub = df[df["dt"] >= tech_launch].copy()
...
era = per[(per["first"] >= tech_launch) & (per["paid"] > 0)].copy()
era["share"] = era["oh"] / era["paid"] * 100
adm_per = df[df["desc"].isin(ADMIN_DESCS)].groupby("record_id")["amount_paid"].sum()
era["adm"] = adm_per.reindex(era.index).fillna(0)
...
out["eraOver20Pct"] = float((era["share"] > 20).mean() * 100)
out["eraAdminPct"] = float((era["adm"] > 0).mean() * 100)
...
# on SEPARATE invoices (distinct billing timestamps). Tech-fee lines excluded
# (one legitimately appears per invoice, and one permit can get several
# invoices in a day). Two corrections the open dataset requires:
# 1. Only PAID lines count: the dataset carries voided or reissued invoice
#    lines with paid=0, and an unpaid twin of a paid line is a voided attempt,
#    not an excess payment. We count those excluded twins for the footnote.
# 2. Identical lines within ONE invoice are collapsed first: the dataset
#    lists per-item lines the records-request extract collapsed (a sign
#    permit for four identical signs carries four identical paid lines on
#    one invoice, verified against invoicenum on the live dataset). Those
#    are line items, not repeats.
d = df[(df["amount_paid"] > 0) & (df["desc"] != TECH)]
d = d.drop_duplicates(subset=["record_id", "date_invoiced", "desc", "amount_paid"])
...
# Voided reissues that a naive billed-based dup count would have flagged:
# unpaid lines whose permit + day + description + amount match a paid line.
paid_day_keys = set(
    zip(d["record_id"], d["day"], d["desc"], d["amount_paid"])
)
u = df[(df["amount_paid"] == 0) & (df["amount_due"] > 0) & (df["desc"] != TECH)]
u = u.drop_duplicates(subset=["record_id", "date_invoiced", "desc", "amount_due"])
u_twin = [
    (r, dy, ds, a) in paid_day_keys
    for r, dy, ds, a in zip(u["record_id"], u["day"], u["desc"], u["amount_due"])
]
junk["dups"] = {
    ...
    "voidedTwinLines": int(sum(u_twin)),
    "voidedTwinDollars": float(u.loc[u_twin, "amount_due"].sum()),
}
print(
    f"dups: {junk['dups']['extraLines']} paid extra lines, "
    f"${junk['dups']['extraPaid']:,.0f} excess paid; excluded "
    f"{junk['dups']['voidedTwinLines']} unpaid voided-reissue twins "
    f"(${junk['dups']['voidedTwinDollars']:,.0f} never paid)"
)
...
junk["ev"] = {
    "lines": int(len(ev)),
    "lines2026": int((ev["year"] == 2026).sum()),
...
# How many descriptions appeared for the first time in 2026 (fee schedule
# churn). Checked against the full history back to 2005, not just the window.
d26 = set(df_all.loc[df_all["year"] == 2026, "desc"].unique())
dpre = set(df_all.loc[df_all["year"] < 2026, "desc"].unique())
